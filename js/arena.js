...
class NightSchoolArenaCMS {
  constructor(channel) {
    ...
    this.contents = null;
    this.setup().then((r) =>
      console.log("ArenaCMS setup complete for", this.channel.name)
    );
  }
  ...
  handleAboutContent(contents) {
    const content = contents.find(
      (content) => content.title === this.channel.name
    );
  ...
  handleReadingListContent(contents) {
    ...
    for (const content of contents) {
      ...
      if (content && content.class === "Attachment") {
        link.href = content.attachment.url;
      } else if (content && content.class === "Media") {
  ...
  handleFamilyDinnerContent(contents) {
    const familyDinnerContent = document.querySelector(this.channel.container);
    for (const content of contents) {
      if (!content && content.class !== "Text") return;
      ...
      const title = document.createElement("h2");
      title.classList.add("dish-date");
      title.innerHTML = content.title;
      const description = document.createElement("p");
      description.classList.add("dish-name");
      description.innerHTML = content.content_html;
      ...
      familyDinnerContent.appendChild(dishContainer);
    }
  }
  ...
  handleNameContent(content) {
    ...
    switch (this.channel.name) {
      ...
      case "Reading List":
        ...
        break;
        case "Family Dinner":
            this.handleFamilyDinnerContent(content);
            break;
    }
  }
}
